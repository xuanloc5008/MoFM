...
def main():
    ...
    acdc_test_slices = collect_acdc_slices(cfg["paths"]["acdc_root"], split="testing")
    acdc_test_ds  = SliceDataset(acdc_test_slices, transforms=val_tf)
    ...
    try:
        mnm1_slices = collect_mnm1_slices(
            cfg["paths"]["mnm1_root"], splits=["Testing", "Validation"]
        )
        ...
        for vendor, indices in vendor_groups.items():
            ...
            v_ds = DomainShiftDataset(vendor_slices, transforms=val_tf)
    ...
    try:
        mnm2_slices = collect_mnm2_slices(cfg["paths"]["mnm2_root"])
        mnm2_ds = DomainShiftDataset(mnm2_slices, transforms=val_tf)
