...
def build_config(args) -> Config:
    ...
    if args.sam_checkpoint:
        config.sam.checkpoint = args.sam_checkpoint
...
def load_experts(config: Config, seed: int, device: torch.device) -> list:
    """Load trained experts from checkpoints."""
    experts = []
    ...
    for expert_id in range(config.gating.num_experts):
        expert = build_sam_expert(
            ...
            expert_id=expert_id,
            primary_structure="LV",  # Dummy — all experts are general-purpose
        ...
        if ckpt_path.exists():
            ...
            print(f"Loaded Expert {expert_id} from {ckpt_path} "
                  f"(val_dice={ckpt['val_dice']:.4f}, σ={ckpt.get('sigma', {})})")
        else:
...
def run_phase3_eval(config, seed, dataset_name="acdc"):
    ...
    train_ds = ACDCDataset(config.train.cache_dir, split="train", augment=False)
    ...
    if dataset_name == "acdc":
        test_ds = ACDCDataset(config.train.cache_dir, split="test", augment=False)
    elif dataset_name == "mms":
        test_ds = ACDCDataset(config.data.mms_root, split="test", augment=False)
    else:
        test_ds = ACDCDataset(config.data.mms2_root, split="test", augment=False)
