...
class ExpertLoss(nn.Module):
    """Total loss for one general-purpose Expert.
    
    All experts use identical loss. Specialisation is NOT enforced here —
    it emerges from the gating network (Phase 2) which routes pixels
    to the expert with lowest uncertainty for that region.
    
    Uncertainty weighting (Kendall et al.) learns task-level difficulty:
    σ_s reflects how inherently hard structure s is, NOT expert specialisation.
    """
    ...
    def forward(self, outputs: Dict, batch: Dict, epoch: int = 0,
        ...
        # 1. Structure losses with uncertainty weighting (Eq. 8)
        # σ_s learns structure difficulty, identical across experts
        ...
        # 2. EDL loss (ramp up over first epochs so segmentation dominates early)
        edl_scale = min(1.0, epoch / max(self.edl_config.kl_anneal_epochs, 1))
        ...
        # Total
        total = (L_struct_uw
                 + self.config.lambda_edl * edl_scale * L_edl
